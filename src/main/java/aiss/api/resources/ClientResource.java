...
public class ClientResource {
	...
	/* Singleton */
	private static ClientResource _instance=null;
	...
	{
		Client list = repository.getPlaylist(id);
	...
	@Produces("application/json")
	public Response addPlaylist(@Context UriInfo uriInfo, Client playlist) {
	...
	@PUT
	@Consumes("application/json")
	public Response updatePlaylist(Client playlist) {
		Client oldplaylist = repository.getPlaylist(playlist.getId());
		if (oldplaylist == null) {
	...
	public Response removePlaylist(@PathParam("id") String id) {
		Client toberemoved=repository.getPlaylist(id);
		if (toberemoved == null)
			throw new NotFoundException("The playlist with id="+ id +" was not found");
		else
	...
	{				
		...
		Client playlist = repository.getPlaylist(playlistId);
